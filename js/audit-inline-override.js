...
// ============================================================
                        ...
                        function auditPersonKey(r) {
                            ...
                            const nameKey = auditNormText(r && r.n);
                            const paKey = auditNormText(r && r.pa);
                            const empKey = auditNormText(r && r.e);
                            if (nameKey) return `N:${nameKey}|${paKey}|${empKey}`;
                            const code = String(r && r.c || '').trim().toUpperCase();
                            if (code && code !== 'NAN' && code !== 'NAT' && code !== '0') return `C:${code}|${paKey}|${empKey}`;
                            return '';
                        ...
                        function auditValidPersonCode(value) {
                            const code = String(value || '').trim().toUpperCase();
                            return !!code && !['NAN', 'NAT', '0', '-', 'PENDIENTE', 'SIN CODIGO'].includes(code);
                        }

                        function auditSameScope(a, b) {
                            const paA = auditNormText(a && a.pa);
                            const paB = auditNormText(b && b.pa);
                            const eA = auditNormText(a && a.e);
                            const eB = auditNormText(b && b.e);
                            return (!paA || !paB || paA === paB) && (!eA || !eB || eA === eB);
                        }

                        function auditRowsSamePerson(a, b) {
                            if (!a || !b || !auditSameScope(a, b)) return false;
                            const codeA = String(a.c || '').trim().toUpperCase();
                            const codeB = String(b.c || '').trim().toUpperCase();
                            if (auditValidPersonCode(codeA) && auditValidPersonCode(codeB) && codeA === codeB) return true;
                            const nameA = auditNormText(a.n || a.nombre || '');
                            const nameB = auditNormText(b.n || b.nombre || '');
                            if (!nameA || !nameB) return false;
                            if (nameA === nameB) return true;
                            return Math.min(nameA.length, nameB.length) >= 14 && (nameA.includes(nameB) || nameB.includes(nameA));
                        }

                        function auditMapHasPerson(map, row) {
                            if (!map || !row) return false;
                            const directKey = auditPersonKey(row);
                            if (directKey && map.has(directKey)) return true;
                            for (const existing of map.values()) {
                                if (auditRowsSamePerson(existing, row)) return true;
                            }
                            return false;
                        }

                        function auditDeletePerson(map, row) {
                            if (!map || !row) return;
                            const directKey = auditPersonKey(row);
                            if (directKey && map.has(directKey)) map.delete(directKey);
                            Array.from(map.entries()).forEach(([key, existing]) => {
                                if (auditRowsSamePerson(existing, row)) map.delete(key);
                            });
                        }
                        ...
                        function buildAuditPeopleBridge(filters) {
                            ...
                            bajasBridgeMap.forEach((v, k) => auditDeletePerson(expectedMap, v));



                            const actualNetMap = new Map(currentMap);

                            bajasCurrentMap.forEach((v, k) => auditDeletePerson(actualNetMap, v));
                            ...
                            expectedMap.forEach((r, k) => {
                                ...
                                if (!auditMapHasPerson(actualNetMap, r)) {
                            ...
                            actualNetMap.forEach((r, k) => {
                                ...
                                if (!auditMapHasPerson(expectedMap, r)) {
                            ...
                            bajasBridgeMap.forEach((b, k) => {
                                ...
                                if (auditMapHasPerson(actualNetMap, b)) {
                                    ...
                                    bajaStillActive.push({
                                        ...
                                        row: actualNetMap.get(k) || Array.from(actualNetMap.values()).find(v => auditRowsSamePerson(v, b)) || b,
                            ...
                            prevMap.forEach((r, k) => {
                                ...
                                if (!auditMapHasPerson(currentMap, r) && !auditMapHasPerson(bajasBridgeMap, r)) {
                            ...
                            currentMap.forEach((r, k) => {
                                ...
                                if (!auditMapHasPerson(prevMap, r) && !auditMapHasPerson(hiresMap, r)) {
                            ...
                            const netDiff = actualNetMap.size - bridgeExpected;
                        ...
                        function renderAudit() {
                            ...
                            try {
                                ...
                                if (tab === 'recon') {
                                    ...
                                    if (headerDiffEl) {
                                        const diffTxt = peopleBridge ? `${peopleBridge.counts.netDiff > 0 ? '+' : ''}${peopleBridge.counts.netDiff}` : '0';
                                        headerDiffEl.textContent = `Dif. ${diffTxt}`;
                                        const hasGap = peopleBridge && Number(peopleBridge.counts.netDiff || 0) !== 0;
                                        headerDiffEl.style.background = hasGap ? '#fee2e2' : '#dcfce7';
                                        headerDiffEl.style.color = hasGap ? '#991b1b' : '#166534';
                                    }
                              ...
                              if (c.netDiff < 0) return (r.diff < 0 ? (r.altas_names || []) : []);
                                                ...
                                                fallbackRows.forEach(r => {
                                                    pickNames(r).forEach(name => {
                                                        ...
                                                        fallback.push({
                                      kind: c.netDiff > 0 ? 'SOBRA EN HC NETO' : 'FALTA EN HC NETO',
                                      ...
                                      detail: c.netDiff > 0 ? ('ESTA DE BAJA DE MAS EN BAJAS DE ' + ((personMatch.e || r.e || 'EMPRESA') + ' ' + (personMatch.pa || r.pa || '')).trim() + '. Revisar si la baja esta duplicada o aplicada de mas.') : ('FALTA EN PLANILLA CONSOLIDADA / HC NETO EN ' + ((personMatch.e || r.e || 'EMPRESA') + ' ' + (personMatch.pa || r.pa || '')).trim() + '. Revisar altas del mes.')
                                                        });
                                                    });
                                                });
                                                if (fallback.length) netIssueRows = [...netIssueRows, ...fallback].slice(0, netIssueTarget);
                                            }
                                            const netIssueLabel = c.netDiff ? 'Diferencias por persona' : 'Sin diferencia neta';
                                            const netIssueTableRows = netIssueRows.map((x, i) => {
                          const mes = peopleBridge.labels.current || x.monthLabel || '-';
...
const rowHtml = (items) => items.length ? items.map((x) => {
                                                ...
                                                const initials = (x.row.n || 'N/A').split(' ').map(w => w.charAt(0)).slice(0, 2).join('').toUpperCase();

                                                return `<article class="audit-person-card ${tone}">

                                                  <div class="audit-person-avatar-wrap">

                                                    <div class="audit-avatar">${initials}</div>

                                                  </div>

                                                  <div class="audit-person-content">

                                                    <div class="audit-person-status-row">
                                                    ...
                                                    <div class="audit-person-meta">
                                                      ...
                                                      <span><i class="fa-solid fa-earth-americas"></i> ${x.row.pa || '-'}</span>

                                                      <span><i class="fa-solid fa-building"></i> ${x.row.e || '-'}</span>

                                                      <span><i class="fa-solid fa-briefcase"></i> ${x.row.p || '-'}</span>
                                            ...
                                            }).join('') : `<div class="audit-empty-row"><i class="fa-solid fa-circle-check"></i> Sin casos en esta categoria.</div>`;



                                            return `

                                            <section class="audit-people-board" style="background:linear-gradient(135deg,#ffffff,#f8fafc);border:1px solid rgba(99,102,241,0.16);border-radius:20px;padding:16px;box-shadow:0 18px 42px rgba(15,23,42,0.08);display:flex;flex-direction:column;gap:14px;">
                                              <div class="audit-equation-card" style="margin:0;box-shadow:none;border-radius:16px;border:1px solid rgba(99,102,241,0.12);">
                                                <div class="audit-section-kicker"><i class="fa-solid fa-arrows-split-up-and-left"></i> Puente de reconciliaci&oacute;n</div>
                                                <div class="audit-equation-flow">
                                                  ...
                                                  <div class="audit-eq-op"><span class="audit-eq-circle">+</span></div>
                                              ...
                                              <div class="audit-conclusion-card ${c.netDiff ? 'has-gap' : 'ok'}" style="margin:0;box-shadow:none;border-radius:16px;">

                                                <div class="audit-conclusion-left">

                                                  <div class="audit-section-kicker"><i class="fa-solid fa-circle-info"></i> ConclusiÃƒÂ³n del cuadre</div>

                            <h3>${c.netDiff ? `${c.netDiff > 0 ? 'Sobran' : 'Faltan'} ${Math.abs(c.netDiff)} ${Math.abs(c.netDiff) === 1 ? 'persona neta' : 'personas netas'}` : 'Cuadre perfectamente cerrado'}</h3>

                            <p>${c.currentNet} HC neto - ${c.bridgeExpected} esperado = ${c.netDiff > 0 ? '+' : ''}${c.netDiff} diferencia neta.</p>
                                              ...
                                              <div id="auditPeopleIssues" class="audit-net-table-card" style="background:#fff;border:1px solid rgba(99,102,241,0.14);border-radius:16px;padding:14px;box-shadow:none;">
                                        ...
                                        <table style="width:100%;border-collapse:collapse;font-size:11px;">
                                          ...
                                          <tbody>
                                            ...
                                            ${allSorted.map((r,i) => {
                                              ...
                                              const rowBridge = buildAuditPeopleBridge({ fMes: String(months[r.m] || '') + ' ' + String(r.y || ''), fEmp: r.e, fPa: r.pa, fAn: String(r.y || '') });

                                              const rowDiff = rowBridge ? Number(rowBridge.counts.netDiff || 0) : Number(r.diff || 0);

                                              const rowHcReal = rowBridge && r.hc_esp != null ? Number(r.hc_esp || 0) + rowDiff : (rowBridge ? rowBridge.counts.currentNet : r.hc_real);

                                              const hasDiff = rowDiff !== 0;

                                              const diffColor = rowDiff > 0 ? '#22c55e' : '#ef4444';

                                              const rowBg = hasDiff ? (i%2===0?'rgba(239,68,68,0.05)':'rgba(239,68,68,0.02)') : (i%2===0?'rgba(0,0,0,0.02)':'transparent');

                                              const altasNames = (r.altas_names||[]).slice(0,5).join(', ') + ((r.altas_names||[]).length>5 ? ` +${r.altas_names.length-5} mÃƒÂ¡s`:'');

                                              const bajasNames = (r.bajas_names||[]).slice(0,5).join(', ') + ((r.bajas_names||[]).length>5 ? ` +${r.bajas_names.length-5} mÃƒÂ¡s`:'');
                                              ...
                                              return `<tr style="background:${rowBg};border-bottom:1px solid rgba(0,0,0,0.04);">
                                                ...
                                                <td style="padding:7px 8px;text-align:center;font-weight:900;color:#6366f1;">${rowHcReal}</td>

                                                <td style="padding:7px 8px;text-align:center;">

                                                  ${hasDiff ? `<span style="background:${diffColor};color:#fff;padding:2px 10px;border-radius:20px;font-weight:900;font-size:12px;">${rowDiff > 0 ? '+' : ''}${rowDiff}</span>` : '<span style="color:#22c55e;font-weight:700;">OK</span>'}
                                ...
                                // ---- SALIDAS VS BAJAS (QUIÃƒâ€°NES FALTAN) ----
                                    ...
                                    <div style="display:flex;gap:15px;margin-bottom:18px;flex-wrap:wrap;align-items:center;">
                                      ...
                                      <div onclick="window._subStatusFilter='SIN_BAJA_REGISTRADA';renderAudit();" style="flex:1;min-width:150px;background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.2);${window._subStatusFilter==='SIN_BAJA_REGISTRADA'?'border-bottom:'+activeBorder:''};border-radius:12px;padding:12px;text-align:center;cursor:pointer;transition:all 0.2s;">
                                        ...
                                        <div style="font-size:24px;font-weight:900;color:#ef4444;">Ã¢Å¡Â Ã¯Â¸Â ${sinBajaCount}</div>
                                      ...
                                      <div onclick="window._subStatusFilter='CON_BAJA_DESFASE';renderAudit();" style="flex:1;min-width:150px;background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.2);${window._subStatusFilter==='CON_BAJA_DESFASE'?'border-bottom:'+activeBorder:''};border-radius:12px;padding:12px;text-align:center;cursor:pointer;transition:all 0.2s;">
                                        ...
                                        <div style="font-size:24px;font-weight:900;color:#f59e0b;">Ã¢Å¡Â Ã¯Â¸Â ${desfaseCount}</div>
                                    ...
                                    if (tableEl) {
                                        ...
                                        <table style="width:100%;border-collapse:collapse;font-size:11px;">
                                          ...
                                          <thead><tr style="background:rgba(236,72,153,0.08);position:sticky;top:0;">
                                            ...
                                            <th style="padding:9px 8px;text-align:left;">CÃƒÂ³digo</th>
                                            ...
                                            <th style="padding:9px 8px;text-align:center;">ÃƒÅ¡ltimo Mes Activo</th>

                                            <th style="padding:9px 8px;text-align:center;">Mes Salida</th>

                                            <th style="padding:9px 8px;text-align:center;">Estado AuditorÃƒÂ­a</th>
                                          ...
                                          <tbody>
                                            ...
                                            ${data.sort((a,b)=>a.n.localeCompare(b.n)).map((r,i) => {
                                                ...
                                                if (r.status === 'SIN_BAJA_REGISTRADA') {
                                                    ...
                                                    detail = `<span style="color:#ef4444;font-weight:700;">Ã¢Å¡Â Ã¯Â¸Â Faltante: No existe registro en planilla de bajas</span>`;
                                                    ...
                                                    const gapWord = r.diff_months > 0 ? `tardÃƒÂ­o (+${r.diff_months} mes${r.diff_months!==1?'es':''})` : `anticipado (${r.diff_months} mes${r.diff_months!==-1?'es':''})`;

                                                    detail = `<span style="color:#d97706;font-weight:700;">Ã¢Å¡Â Ã¯Â¸Â Registrada en <b>${r.baja_label}</b> (${gapWord})<br><small style="color:#64748b;font-weight:600;">Fecha baja: ${r.baja_fecha} | Motivo: ${r.baja_motivo || 'No indicado'}</small></span>`;
                                ...
                                // ---- DOBLE PAGO ----
                                    ...
                                    if (subtitle) subtitle.textContent = 'Personas en 2+ empresas el mismo mes Ã¢â‚¬â€ posible doble pago en planilla';
                                    ...
                                    if (tableEl) {
                                        ...
                                        <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:10px 18px;margin-bottom:14px;font-size:12px;color:#991b1b;">
                                          ...
                                          <b>Ã¢Å¡Â Ã¯Â¸Â ALERTA:</b> Estas ${data.length} personas aparecen en 2+ empresas el mismo mes. Puede ser un error de registro o un doble pago real.
                                        ...
                                        <table style="width:100%;border-collapse:collapse;font-size:11px;">
                                          ...
                                          <thead><tr style="background:rgba(239,68,68,0.08);position:sticky;top:0;">
                                            ...
                                            <th style="padding:9px 8px;text-align:left;">Empresas simultÃƒÂ¡neas</th>
                                          ...
                                          <tbody>
                                            ...
                                            ${data.map((d,i)=>`<tr style="background:${i%2===0?'rgba(239,68,68,0.03)':'transparent'};border-bottom:1px solid rgba(239,68,68,0.06);">
                                              ...
                                              <td style="padding:7px 8px;font-weight:700;color:#ef4444;">Ã¢Å¡Â Ã¯Â¸Â ${d.n}</td>
                                        ...
                                        </table>` : '<p style="color:#22c55e;padding:30px;text-align:center;">Ã¢Å“â€¦ Sin alertas de doble pago en el filtro seleccionado</p>';
                        ...
                        function auditClearFilters() {
                            ['auditFilterAnio', 'auditFilterMes', 'auditFilterEmpresa', 'auditFilterPa'].forEach((id) => {
                                const el = document.getElementById(id);
                                if (el) el.value = '';
                            });
                            window._subStatusFilter = '';
...
window.auditSetTab = auditSetTab;
...
window.renderSustituciones = renderSustituciones;


setTimeout(() => {
  try {
    if (document.getElementById('auditTable')) renderAudit();
  } catch (err) {
    console.error('audit override initial render error:', err);
    const table = document.getElementById('auditTable');
    if (table) table.innerHTML = '<div class="audit-empty" style="padding:18px;color:#ef4444;font-weight:900;">No se pudo cargar Auditoria. Revisa consola para detalle.</div>';
  }
}, 0);
})();
