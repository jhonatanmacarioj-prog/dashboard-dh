...
function renderCostos(s, emps) {
                ...
                if (needsFullRender) {
                    ...
                    pane.innerHTML = `
                        ...
                        <div class="card-box cost-chart-card cost-wide-card">
                            ...
                            <div class="cost-chart-stage cost-chart-stage-lg"><canvas id="chartRegStacked"></canvas></div>
                        </div>
                        <div class="cost-grid-duo">
                            <div class="card-box cost-chart-card cost-panel-dark">
                                ...
                                <div class="cost-donut-layout">
                                    <div id="costBreakdownLegend" class="cost-donut-legend"></div>
                            ...
                            <div class="card-box cost-chart-card cost-panel-dark">
                                ...
                                <div class="cost-donut-layout">
                                    <div id="costDirLegend" class="cost-donut-legend"></div>
                        ...
                        <div class="card-box cost-chart-card cost-wide-card">
                            <div class="cost-card-head">
                                ...
                                <div class="cost-mini-toggle">
                                    <button id="btnCostTrend6" onclick="window._costViewLength=6; renderAll();">6 Meses</button>
                                    <button id="btnCostTrend12" onclick="window._costViewLength=12; renderAll();">12 Meses</button>
                                </div>
                            </div>
                            <div id="costPayrollTrendSummary" class="cost-inline-metrics"></div>
                            <div class="cost-chart-stage cost-chart-stage-lg"><canvas id="chartPayrollTrend"></canvas></div>
                        </div>
                        <div class="card-box cost-chart-card cost-wide-card">
                            <div class="cost-card-head">
                                <div>
                            ...
                            <div id="costVarTrendSummary" class="cost-inline-metrics"></div>
                        ...
                        <div id="costVariableInsights" class="cost-grid-split cost-grid-split-tight">
                            <div class="card-box cost-chart-card">
                                ...
                                <div id="costTopVariableSummary" class="cost-inline-metrics"></div>
                ...
                `;
                ['btnCost6m','btnCost12m','btnCost1y','btnCostTrend6','btnCostTrend12'].forEach(id => {
                    const btn = document.getElementById(id);
                    if (btn) btn.classList.toggle('active', ((id === 'btnCost6m' || id === 'btnCostTrend6') && spanMonths === 6) || ((id === 'btnCost12m' || id === 'btnCost1y' || id === 'btnCostTrend12') && spanMonths === 12));
                });
                ...
                if (ctxReg) {
                    ...
                    window.activeCharts.push(new Chart(ctxReg, {
                        ...
                        data: {
                            ...
                            datasets: [
                                { label: 'Salario Base', data: sortedReg.map(r => r[1].s), backgroundColor: gradSO, borderColor: '#5b21b6', borderWidth: 1, borderSkipped: false, borderRadius: 8, barPercentage: 0.84, categoryPercentage: 0.76, datalabels: { display: false } },
                                { label: 'Bonos/Extra', data: sortedReg.map(r => r[1].b), backgroundColor: gradB, borderColor: '#155e75', borderWidth: 1, borderSkipped: false, borderRadius: 8, barPercentage: 0.84, categoryPercentage: 0.76, datalabels: { display: false } },
                                { label: 'Cargas/Prov', data: sortedReg.map(r => r[1].c), backgroundColor: gradC, borderColor: '#92400e', borderWidth: 1, borderSkipped: false, borderRadius: 8, barPercentage: 0.84, categoryPercentage: 0.76 }
                            ]
                        },
                        options: {
                            ...tOpts,
                            ...
                            layout: { padding: { right: 185, left: 8, top: 8, bottom: 4 } },
                            plugins: {
                                ...tOpts.plugins,
                                legend: { position: 'top', align: 'start', labels: { color: '#172033', boxWidth: 12, boxHeight: 12, usePointStyle: true, pointStyle: 'rectRounded', font: { family: 'Montserrat', size: 12, weight: '900' } } },
                                tooltip: { ...tOpts.plugins.tooltip, callbacks: { label: ctx => ' ' + ctx.dataset.label + ': ' + fM(ctx.raw), footer: items => 'Total país: ' + fM(items.reduce((sum, item) => sum + (Number(item.parsed.x) || 0), 0)) } },
                                datalabels: {
                                    ...
                                    clip: false,
                                    color: '#111827',
                                    formatter: (value, ctx) => fM(sortedReg[ctx.dataIndex]?.[1]?.t || 0),
                                    font: { family: 'Montserrat', size: 12, weight: '900' },
                                    textStrokeColor: '#ffffff',
                                    textStrokeWidth: 3
                                }
                            },
                            scales: {
                                x: { stacked: true, border: { color: '#cbd5e1' }, grid: { color: 'rgba(71, 85, 105, 0.10)', drawTicks: false }, ticks: { color: '#475569', padding: 8, font: { family: 'Montserrat', size: 11, weight: '800' }, callback: v => costShort(v, currency) } },
                                y: { stacked: true, border: { display: false }, grid: { display:false }, ticks: { color: '#0f172a', padding: 12, font: { family: 'Montserrat', size: 14, weight: '900' } } }
                            }
                        }
                    }));
                }
                ...
                if (ctxBD) {
                    const bdLabels = ['Salarios', 'Variables', 'Patronales', 'Aguinaldo/B14', 'Vacaciones/Ind'];
                    const bdValues = [tSO, tVar, tPat, (tAgui + tB14), (tVac + tInd)];
                    const bdColors = ['#8b5cf6', '#ec4899', '#f59e0b', '#10b981', '#64748b'];
                    const bdTotal = bdValues.reduce((sum, val) => sum + (Number(val) || 0), 0);
                    const bdLegend = document.getElementById('costBreakdownLegend');
                    if (bdLegend) bdLegend.innerHTML = bdLabels.map((label, i) =>
                        '<div class="cost-donut-legend-row">' +
                        '<span class="cost-donut-legend-dot" style="background:' + bdColors[i] + '"></span>' +
                        '<span>' + label + '</span>' +
                        '<b>' + fM(bdValues[i] || 0) + '</b>' +
                        '</div>'
                    ).join('');
                    window.activeCharts.push(new Chart(ctxBD, {
                        type: 'doughnut',
                        plugins: window.ChartDataLabels ? [ChartDataLabels] : [],
                        data: { labels: bdLabels, datasets: [{ data: bdValues, backgroundColor: bdColors, borderWidth: 2, borderColor: '#ffffff', hoverOffset: 8 }] },
                        options: {
                            ...cOpts,
                            cutout: '66%',
                            plugins: {
                                legend: { display: false },
                                tooltip: { callbacks: { label: ctx => ctx.label + ': ' + fM(ctx.raw) + ' (' + (bdTotal ? ((ctx.raw / bdTotal) * 100).toFixed(1) : '0.0') + '%)' } },
                                datalabels: {
                                    display: ctx => bdTotal > 0 && (((ctx.dataset.data[ctx.dataIndex] || 0) / bdTotal) >= 0.055),
                                    color: '#ffffff',
                                    formatter: (value, ctx) => String(ctx.chart.data.labels[ctx.dataIndex] || '').toUpperCase(),
                                    font: { family: 'Montserrat', size: 10, weight: '900' },
                ...
                if (ctxPT) {
                    const lastCostPoint = costTrendForAvg[costTrendForAvg.length - 1] || 0;
                    const lastCostPrev = priorCostTrend[priorCostTrend.length - 1] || 0;
                    const maxCostIdx = costTrendForAvg.reduce((best, value, idx, arr) => value > (arr[best] || 0) ? idx : best, 0);
                    const payrollSummary = document.getElementById('costPayrollTrendSummary');
                    if (payrollSummary) payrollSummary.innerHTML = `
                        <div class="cost-inline-metric">
                            <span>Promedio mensual</span>
                            <b>${fM(avgCost || 0)}</b>
                            <small>${spanMonths} meses visibles</small>
                        </div>
                        <div class="cost-inline-metric">
                            <span>Último mes</span>
                            <b>${fM(lastCostPoint)}</b>
                            <small>Anterior: ${fM(lastCostPrev)}</small>
                        </div>
                        <div class="cost-inline-metric">
                            <span>Pico</span>
                            <b>${trendLabels[maxCostIdx] || '-'}</b>
                            <small>${fM(costTrendForAvg[maxCostIdx] || 0)}</small>
                        </div>
                    `;
                    ...
                    window.activeCharts.push(new Chart(ctxPT, {
                        ...
                        options: {
                            ...
                            layout: { padding: { top: 28, right: 18, left: 8, bottom: 10 } },
                            plugins: {
                                ...costChartBase(currency).plugins,
                                legend: { position: 'top', align: 'start', labels: { color: '#334155', boxWidth: 12, boxHeight: 12, usePointStyle: true, pointStyle: 'circle', font: { family: 'Montserrat', size: 11, weight: '800' } } },
                                ...
                                datalabels: {
                                    ...
                                    offset: 8,
                                    color: '#111827',
                                    formatter: value => costShort(value, currency),
                                    font: { family: 'Montserrat', size: 11, weight: '900' },
                                    textStrokeColor: '#ffffff',
                                    textStrokeWidth: 3
                                }
                            },
                            scales: {
                                x: {
                                    grid: { display: false },
                                    ticks: { color: '#475569', font: { family: 'Montserrat', size: 11, weight: '800' } }
                                },
                                y: {
                                    grid: { color: 'rgba(148, 163, 184, 0.10)' },
                                    ticks: { color: '#475569', font: { family: 'Montserrat', size: 11, weight: '800' }, callback: v => costShort(v, currency) }
                                }
                            }
                        }
                    }));
                ...
                if (ctxDM) {
                    ...
                    const dirLabels = sorted.map(s => costAxisLabel(s[0], 24));
                    const dirValues = sorted.map(s => s[1]);
                    const dirColors = ['#8b5cf6', '#10b981', '#f59e0b', '#ec4899', '#06b6d4', '#475569'];
                    const dirTotal = dirValues.reduce((sum, val) => sum + (Number(val) || 0), 0);
                    const dirLegend = document.getElementById('costDirLegend');
                    if (dirLegend) dirLegend.innerHTML = dirLabels.map((label, i) =>
                        '<div class="cost-donut-legend-row">' +
                        '<span class="cost-donut-legend-dot" style="background:' + dirColors[i % dirColors.length] + '"></span>' +
                        '<span>' + label + '</span>' +
                        '<b>' + fM(dirValues[i] || 0) + '</b>' +
                        '</div>'
                    ).join('');
                    window.activeCharts.push(new Chart(ctxDM, {
                        type: 'doughnut',
                        plugins: window.ChartDataLabels ? [ChartDataLabels] : [],
                        data: { labels: dirLabels, datasets: [{ data: dirValues, backgroundColor: dirLabels.map((_, i) => dirColors[i % dirColors.length]), borderWidth: 2, borderColor: '#ffffff', hoverOffset: 8 }] },
                        options: {
                            ...cOpts,
                            cutout: '66%',
                            plugins: {
                                legend: { display: false },
                                tooltip: { callbacks: { label: ctx => ctx.label + ': ' + fM(ctx.raw) + ' (' + (dirTotal ? ((ctx.raw / dirTotal) * 100).toFixed(1) : '0.0') + '%)' } },
                                datalabels: {
                                    display: ctx => dirTotal > 0 && (((ctx.dataset.data[ctx.dataIndex] || 0) / dirTotal) >= 0.065),
                                    color: '#ffffff',
                                    formatter: (value, ctx) => String(ctx.chart.data.labels[ctx.dataIndex] || '').toUpperCase(),
                                    font: { family: 'Montserrat', size: 10, weight: '900' },
                                    textStrokeColor: 'rgba(15, 23, 42, 0.35)',
                ...
                if (ctxVT) {
                    ...
                    const lastVarPoint = varTrend[varTrend.length - 1] || 0;
                    const lastVarPrev = priorVarTrend[priorVarTrend.length - 1] || 0;
                    const avgVarCurrent = spanMonths ? totalVar / spanMonths : 0;
                    const varSummary = document.getElementById('costVarTrendSummary');
                    if (varSummary) varSummary.innerHTML = `
                        <div class="cost-inline-metric accent-pink">
                            <span>Total visible</span>
                            <b>${fM(totalVar)}</b>
                            <small>${spanMonths} meses</small>
                        </div>
                        <div class="cost-inline-metric accent-pink">
                            <span>Último mes</span>
                            <b>${fM(lastVarPoint)}</b>
                            <small>Anterior: ${fM(lastVarPrev)}</small>
                        </div>
                        <div class="cost-inline-metric accent-pink">
                            <span>Pico variable</span>
                            <b>${trendLabels[peakIdx] || '-'}</b>
                            <small>${fM(varTrend[peakIdx] || 0)}</small>
                        </div>
                    `;
                    ...
                    window.activeCharts.push(new Chart(ctxVT, {
                            ...
                            datasets: [
                                {
                                    ...
                                    datalabels: {
                                        ...
                                        offset: 6,
                                        color: '#111827',
                                        formatter: value => value > 0 ? costShort(value, currency) : '',
                                        font: { family: 'Montserrat', size: 10, weight: '900' },
                                        textStrokeColor: '#ffffff',
                                        textStrokeWidth: 3
                                    }
                                },
                                {
                                    type: 'line',
                                    label: 'Promedio',
                                    data: varTrend.map(() => avgVarCurrent),
                                    borderColor: '#475569',
                                    borderDash: [6, 6],
                                    borderWidth: 2,
                                    pointRadius: 0,
                        ...
                        options: {
                            ...costChartBase(currency),
                            plugins: {
                                legend: { position: 'top', align: 'start', labels: { color: '#334155', boxWidth: 12, boxHeight: 12, usePointStyle: true, pointStyle: 'rectRounded', font: { family: 'Montserrat', size: 11, weight: '800' } } },
                                tooltip: { callbacks: { label: ctx => `${ctx.dataset.label}: ${fM(ctx.raw)}` } }
                            },
                            scales: {
                                x: { grid: { display: false }, ticks: { color: '#475569', font: { family: 'Montserrat', size: 11, weight: '800' } } },
                                y: { grid: { color: 'rgba(148, 163, 184, 0.10)' }, ticks: { color: '#475569', font: { family: 'Montserrat', size: 11, weight: '800' }, callback: v => costShort(v, currency) } }
                            }
                        }
                    }));
                }
                ...
                if (ctxTopVar) {
                    const topPeople = filtered
                        ...
                        .slice(0, 12);
                    const topLeader = topPeople[0];
                    const topPeopleTotal = topPeople.reduce((sum, item) => sum + (item.val || 0), 0);
                    const topPeopleAvg = topPeople.length ? topPeopleTotal / topPeople.length : 0;
                    const topVarSummary = document.getElementById('costTopVariableSummary');
                    if (topVarSummary) topVarSummary.innerHTML = `
                        <div class="cost-inline-metric accent-gold">
                            <span>Líder actual</span>
                            <b>${topLeader ? (topLeader.e.n || '').split(' ').slice(0, 2).join(' ') : 'Sin data'}</b>
                            <small>${topLeader ? fM(topLeader.val || 0) : '0'}</small>
                        </div>
                        <div class="cost-inline-metric accent-gold">
                            <span>Total top ranking</span>
                            <b>${fM(topPeopleTotal)}</b>
                            <small>${topPeople.length} personas</small>
                        </div>
                        <div class="cost-inline-metric accent-gold">
                            <span>Promedio top</span>
                            <b>${fM(topPeopleAvg)}</b>
                            <small>variable por persona</small>
                        </div>
                    `;
                    ...
                    window.activeCharts.push(new Chart(ctxTopVar, {
                        ...
                        options: {
                            ...costChartBase(currency),
                            indexAxis: 'y',
                            layout: { padding: { right: 80, left: 4, top: 4, bottom: 4 } },
                            plugins: {
                                legend: { display: false },
                                tooltip: {
                            ...
                            scales: {
                                x: { grid: { color: 'rgba(148, 163, 184, 0.10)' }, ticks: { color: '#475569', font: { family: 'Montserrat', size: 11, weight: '800' }, callback: v => costShort(v, currency) } },
                                y: { grid: { display: false }, ticks: { color: '#334155', font: { family: 'Montserrat', size: 11, weight: '800' } } }
                            }
                        }
                    }));
                }
...
window.renderCostosOverride = renderCostos;
